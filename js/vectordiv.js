...
~~~~~~~~~~~~~
 ...
 Fourth Parameter (Optional): The amount to scroll each time the control is clicked. 
   This can be specified as an integer representing pixels, or as a quoted percentage between 1 and
   100, e.g. "40%". The latter specifies 40% of the div's full scrollable distance. 
   Alternatively, the "V" modifier can be used to specify percentage of visible content height/width:
   "40%V".
  
   If the fourth parameter is omitted, the script will scroll the maximum possible distance in the 
...
var VectorDivObj = function( ctrlId, divId, vects, disp )  
  {
    this.ctrl = null;
    this.elem = null;     
    this.xVect = 0;
    ...
    this.go = function( evt )  /** VISIBLE SOURCE DOES NOT MEAN OPEN SOURCE **/
    {
      var inst = this,
          ...
          xPercent = this.useViewPercent ? (this.percent * ( this.elem.offsetWidth / xTravel )) : this.percent,
          yPercent = this.useViewPercent ? (this.percent * (this.elem.offsetHeight / yTravel )): this.percent;
    ...
    this.getDisp = function()
    {
      ...
      if( disp && ( m = disp.toString().replace( /[0-9%V]/ig, "").match( /\S+/ ) ) )
      {
